package br.com.meiraonline.resource.exception;
...
@ControllerAdvice
public class MeiraResourceExceptionHandler {
	
	@ExceptionHandler(MeiraOnlineObjectNotFoundException.class)
	public ResponseEntity<MeiraStandardError> objectNotFound(MeiraOnlineObjectNotFoundException e, HttpServletRequest request) {
		
		MeiraStandardError meiraStandardError = new MeiraStandardError(System.currentTimeMillis(), HttpStatus.NOT_FOUND.value(), "Not found", e.getMessage(), request.getRequestURI());
		return ResponseEntity.status(HttpStatus.NOT_FOUND).body(meiraStandardError);
	}
